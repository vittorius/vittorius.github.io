...
**The saved choice expires after 6 hours.** `setTheme` also writes `localStorage["theme_saved_at"] = Date.now()`, and the head script clears both keys (reverting to `prefers-color-scheme`) when `theme_saved_at` is missing or older than 6h. Every explicit toggle refreshes the timestamp, so only 6h of inactivity resets the choice; a legacy `theme` value with no timestamp is treated as stale and cleared on first load. The check lives in the pre-paint head script so the revert applies before first paint (no flash).
